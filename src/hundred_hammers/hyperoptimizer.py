...
class HyperOptimizer(ABC):
    ...
    def __init__(self, metric: callable, metric_params: dict):
        self.metric = make_scorer(lambda y_true, y_pred: metric(y_true, y_pred, **metric_params))

    def set_metric(self, metric: callable, metric_params: dict):
        """
...
class HyperOptimizerGridSearch(HyperOptimizer):
    ...
    def __init__(self, metric: callable, metric_params: dict, n_folds_tune: int = 5, n_grid_points: int = 10):
    ...
    def best_params(self, X: np.ndarray, y: np.ndarray, model: BaseEstimator, param_grid: dict = None):
        ...
        with warnings.catch_warnings():
            ...
            grid_search_model = GridSearchCV(model, param_grid, scoring=self.metric, n_jobs=-1, cv=self.n_folds_tune)
...
class HyperOptimizerRandomSearch(HyperOptimizer):
    ...
    def __init__(self, metric: callable, metric_params: dict, n_folds_tune: int = 5, n_iter: int = 10):
        super().__init__(metric, metric_params)
        self.n_folds_tune = n_folds_tune
        self.n_iter = n_iter
    ...
    def best_params(self, X: np.ndarray, y: np.ndarray, model: BaseEstimator, param_grid: dict = None):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            grid_search_model = RandomizedSearchCV(model, param_grid, scoring=self.metric, n_jobs=-1, cv=self.n_folds_tune, n_iter=self.n_iter)
            grid_search_model.fit(X, y)
